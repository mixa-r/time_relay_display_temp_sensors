...
//   ОБЪЯВЛЯЕМ КОНСТАНТЫ КОТОРЫЕ МОЖНО РЕДАКТИРОВАТЬ:                                         //
     const uint8_t   pinButton1   =             11;
     const uint8_t   pinButton2   =             12;
     const uint8_t   pinButton3   =             13;
     const uint8_t   pinButton4   =             3;
...
void funcDisplayUpdate(){                                                                     //
     ...
     /*  "00.00.0000г. ПН " */  lcd.setCursor(0, 1); lcd.print(time.gettime("d.m.Y\5."));     //
                                //lcd.setCursor(13,1); lcd.print(time.weekday==1?"\7H":(time.weekday==2?"BT":(time.weekday==3?"CP":(time.weekday==4?"\7T":(time.weekday==5?"\7T":(time.weekday==6?"C\6":("BC")))))));
...
//************************Режим для 3 кнопки*********************************** 
     if(!digitalRead(pinButton3) == HIGH && flagButton3 == 0)      
    {                                              
      digitalWrite(pinChanel_3, !digitalRead(pinChanel_3));
      flagButton3 = 1;
    }
  
  if(!digitalRead(pinButton3) == LOW && flagButton3 == 1)
    {
      flagButton3 = 0;
    } 
/*//************************Режим для 4 кнопки***********************************
    if(!digitalRead(pinButton4) == HIGH && flagButton4 == 0)      
    {                                              
      digitalWrite(pinChanel_4, !digitalRead(pinChanel_4));
      flagButton4 = 1;
    }
  
  if(!digitalRead(pinButton4) == LOW && flagButton4 == 1)
    {
      flagButton4 = 0;
    } */
    if(sensor.tem > tmax){digitalWrite(pinChanel_4, LOW);}                                 // Настройка реле включение по температуре
    if(sensor.tem < tmin){digitalWrite(pinChanel_4, HIGH);}                                // Настройка реле включение по температуре
}
