...
//ОБЪЯВЛЯЕМ ПЕРЕМЕННЫЕ ТЕМПЕРАТУРЫ ПО КОТОРЫМ БУДЕТ СРАБАТЫВАТЬ РЕЛЕ
     int tgis = 5;                                                                            //  Определяем Гистерезис температуры                   
     #include <iarduino_DHT.h>   // подключаем библиотеку для работы с датчиком DHT
     iarduino_DHT sensor1(A3);                                                                                              //
     iarduino_DHT sensor2(A2);
...
void setup(){                                                                                 //
     ...
     //digitalWrite(pinChanel_1, HIGH);
     //digitalWrite(pinChanel_2, HIGH);
     ...
     //lcd.print(F("PE\1E BPEMEH\2"));                                                          //  Выводим надпись «PEЛE BPEMEHИ». Буквы «Л» и «И» заменяем знаком '\' и №, под которыми их графические представления были загружены в память CGRAM LCD дисплея.
     //lcd.setCursor(2,1);                                                                      //  Устанавливаем курсор в 5 столбец 1 строки (нумерация начинается с 0)
     //lcd.print(F("Добрый день!"));                                                            //  Выводим надпись «iarduino.ru».
     //delay(1000);                                                                             //  Ждём, что бы надпись отображаемую на дисплее успели прочитать
     ...
     if(!funcTestTimer()){                                                                    //  Если значения в EEPROM не соответствуют значениям таймера (например первый запуск данного скетча), то ...
         ...
         lcd.setCursor(0,0);                                                                  //  Устанавливаем курсор в верхний левый угол экрана (0 столбец, 0 строка)
         lcd.print(F("\1O\2\3OTOBKA..."));                                                    //  Выводим надпись «ПOДГOTOBKA...». Буквы «П», «Д» и «Г» заменяем знаком '\' и №, под которыми их графические представления были загружены в память CGRAM LCD дисплея.
...
//  if(sensor.tem >= funcReadTimer(valTimerNum,6)){digitalWrite(pinChanel_4, HIGH);}                                 // Настройка реле включение по температуре
//  if(sensor.tem <= funcReadTimer(valTimerNum,6)-tgis){digitalWrite(pinChanel_4, LOW);} 
}                                                                                             //
                                                                                              //
...
//*************************************************************************************************************************************
     ...
     /*  "      100       " */  if(i==encRIGHT){flgDisplayUpdate=1; valArray[0]+=1; if(valArray[0]>40){valArray[0]=40;}}
         break;   
//*************************************************************************************************************************************     
     }                                                                                        //
...
void funcDisplayUpdate(){                                                                     //
     ...
     /*  "00.00.0000г. ПН " */  //lcd.setCursor(0, 1); lcd.print(time.gettime("d."));     //
                                ...
                                lcd.setCursor(0,1);
                                lcd.print("t1 = ");
                                lcd.print((int)(sensor1.tem));
                                lcd.setCursor(9,1);
                                lcd.print("t2 = ");
                                lcd.print((int)(sensor2.tem));
         break;                                                                               //
...
void funcSetPWM(void){                                                                        //
     ...
     if(valMode==0){                                                                          //  Если текущий режим равен 0 "Вне меню", то ...
         uint8_t  getChanel[4]   = {0,0,0,0};                                                 //  Определяем массив, каждый элемент которого соответствует уровню сигнала (от 0 до 100%) прочитанного из активированного таймера для соответствующего канала
...
void funcTemp2(){
      if(sensor2.tem >= funcReadTimer(valTimerNum,6)){digitalWrite(pinChanel_4, HIGH);}                                 // Настройка реле включение по температуре
      if(sensor2.tem <= funcReadTimer(valTimerNum,6)-tgis){digitalWrite(pinChanel_4, LOW);} 
}

void funcTemp1(){
      if(sensor1.tem >= funcReadTimer(valTimerNum,6)){digitalWrite(pinChanel_3, HIGH);}                                 // Настройка реле включение по температуре
      if(sensor1.tem <= funcReadTimer(valTimerNum,6)-tgis){digitalWrite(pinChanel_3, LOW);} 
}
